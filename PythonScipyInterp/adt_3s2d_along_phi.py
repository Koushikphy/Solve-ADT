...
def ang2diab(enr, ang):
    ...
    def aMat(a,b,c): # 12, 13, 23
        return np.array([
            [cs(a)*cs(b),    sn(a)*cs(c)-cs(a)*sn(b)*sn(c),   cs(a)*sn(b)*cs(c)+sn(a)*sn(c) ],
            [-sn(a)*cs(b),   cs(a)*cs(c)+sn(a)*sn(b)*sn(c),   -sn(a)*sn(b)*cs(c)+cs(a)*sn(c)],
            [-sn(b),        -cs(b)*sn(c),                     cs(b)*cs(c)                   ]
        ])
...
gridPh = tauth[0,:,1]
tauth = tauth[...,2:]
# instanciate the ADT object
ss = adtSolver3S2D(gridTh, gridPh, tauth, tauth)  # sending dummy tauph, as its not required
